...
/**
 * Export J2Store product image fields.
 *
 * @package     CSVI
 * @subpackage  J2Store product images
 * @since       7.3.0
 */
class Productimage extends \CsviModelExports
{
	/**
	 * Export the data.
	 *
	...
	protected function exportBody()
	{
		if (parent::exportBody())
		{
			...
			foreach ($exportFields as $field)
			{
				...
				{
					case 'sku':
						$userFields[] = $this->db->quoteName('#__j2store_productimages.product_id');

						if (array_key_exists($field->field_name, $groupByFields))
						{
							$groupBy[] = $this->db->quoteName('#__j2store_productimages.product_id');
						}

						if (array_key_exists($field->field_name, $sortByFields))
						{
							$sortBy[] = $this->db->quoteName('#__j2store_productimages.product_id');
						}
						break;
					case 'main_image':
					case 'thumb_image':
					...
					case 'j2store_productimage_id':
						$userFields[] = $this->db->quoteName('#__j2store_productimages.' . $field->field_name);

						if (array_key_exists($field->field_name, $groupByFields))
						{
							$groupBy[] = $this->db->quoteName('#__j2store_productimages.' . $field->field_name);
						}

						if (array_key_exists($field->field_name, $sortByFields))
						{
							$sortBy[] = $this->db->quoteName('#__j2store_productimages.' . $field->field_name);
						}
						break;
					case 'custom':
						break;
			...
			$query->from($this->db->quoteName('#__j2store_productimages'));
					...
					foreach ($exportFields as $field)
					{
						...
						switch ($fieldName)
						{
							case 'sku':
								...
								break;
							case 'additional_images':
								$images = json_decode($record->additional_images, true);
