...
class PlgContentPerfectimage extends JPlugin
{
	function onAjaxPerfectimage()
	{
		...
		try
		{
			...
			{
				throw new Exception(JText::_('You are not authorised to upload file.'));
			}
			...
			// Image folder path
			$imagefolder = JPATH_SITE . '/images/' . date('Y') . '/' . date('m') . '/';
			...
			// Filename
			$random   = substr(str_shuffle(md5(time())), 0, 3);
			...
			// Image name
			$image = str_replace(JPATH_SITE . '/', '', $filepath);
