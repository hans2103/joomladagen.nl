...
class JFormFieldPerfectimage extends JFormField
{
	...
	protected function getInput()
	{
		...
		// Select button
		$html[] = '<a href="#' . $this->id . '_modal" role="button" class="btn btn-primary perfect-image-select" data-toggle="modal" title="' . JText::_('JSELECT') . '">'
			. '<span class="icon-list icon-white"></span> '
			. JText::_('JSELECT') . '</a>';

		// Reset button
		$html[] = '<button class="btn perfect-image-clear' . ($this->value ? '' : ' hidden') . '"><span class="icon-remove"></span>' . JText::_('JCLEAR') . '</button>';
		...
		$data = array(
			...
			'max_size_message' => JText::_('Het foto-bestand is te groot. Maak de afbeelding eerst kleiner tot maximaal 1200px bij 1200px (bijvoorbeeld via http://www.fotoverkleinen.nl/) en probeer het dan opnieuw.'),
			'max_dimension' => 15000
		);
		...
		$html[] = JHtml::_('bootstrap.renderModal',
			$this->id . '_modal',
			array(
				'title'  => JText::_('Selecteer afbeelding'),
				'footer' =>
					'<button class="btn" data-dismiss="modal" aria-hidden="true">' . JText::_("JLIB_HTML_BEHAVIOR_CLOSE") . '</button>' .
					'<button class="btn btn-success perfect-image-save" data-dismiss="modal" aria-hidden="true">' . JText::_("JSAVE") . '</button>'
			),
			$layout->render($data)
		);
