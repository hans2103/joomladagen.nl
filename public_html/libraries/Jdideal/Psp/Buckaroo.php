...
class Buckaroo
{
	...
	public function getAvailablePaymentMethods()
	{
		return array(
			...
			'bancontactmrcash'   => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_BANCONTACT'),
			'mastercard'         => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_MASTERCARD'),
			'visa'               => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_VISA'),
			'amex'               => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_AMEX'),
			'sofortueberweisung' => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_SOFORT'),
			'paypal'             => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_PAYPAL'),
			'paysafecard'        => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_PAYSAFECARD'),
			'giropay'            => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_GIROPAY'),
			'sepadirectdebit'    => \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_SEPADIRECTDEBIT'),
		);
	}
		...
		foreach ($selected as $name)
		{
			switch ($name)
			{
				case 'sofortueberweisung':
					$options[] = \JHtml::_('select.option', 'sofortueberweisung', \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_SOFORT'));
					break;
				case 'bancontactmrcash':
					$options[] = \JHtml::_('select.option', 'bancontactmrcash', \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_BANCONTACT'));
					break;
				case 'mastercard':
				...
				case 'visa':
					...
					break;
				case 'amex':
					$options[] = \JHtml::_('select.option', 'amex', \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_AMEX'));
					break;
				case 'paypal':
					$options[] = \JHtml::_('select.option', 'paypal', \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_PAYPAL'));
					break;
				case 'giropay':
					$options[] = \JHtml::_('select.option', 'giropay', \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_GIROPAY'));
					break;
				case 'sepadirectdebit':
					$options[] = \JHtml::_('select.option', 'sepadirectdebit', \JText::_('COM_JDIDEALGATWAY_PAYMENT_METHOD_SEPADIRECTDEBIT'));
					break;
				default:
				case 'ideal':
		...
		if ($statusRequest || $returnData['brq_signature'] === $signature)
		{
			...
			switch ($returnData['brq_statuscode'])
			{
				...
				case '490':
					...
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_490');
					break;
				case '491':
					$status['suggestedAction'] = 'FAILURE';
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_491');
					break;
				case '492':
					$status['suggestedAction'] = 'ERROR';
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_492');
					break;
				case '791':
					$status['suggestedAction'] = 'OPEN';
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_791');
					break;
				case '690':
					$status['suggestedAction'] = 'FAILURE';
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_690');
					break;
				case '890':
					$status['suggestedAction'] = 'CANCELLED';
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_890');
					break;
				default:
					$status['suggestedAction'] = 'FAILURE';
					$status['error_message'] = \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_RESULT_' . $returnData['brq_statuscode']);
					break;
			}
	...
	private function send(Gateway $jdideal, array $keyvalue = null, $operation = null)
	{
		...
		$return = array('BRQ_STATUSMESSAGE' => \JText::_('COM_JDIDEALGATEWAY_BUCKAROO_HASH_FAILED'));
	...
	private function getBanks()
	{
		...
		$options['_']['items'][] = \JHtml::_('select.option', 'ABNANL2A', 'ABN Amro Bank');
		$options['_']['items'][] = \JHtml::_('select.option', 'INGBNL2A', 'ING');
		$options['_']['items'][] = \JHtml::_('select.option', 'KNABNL2H', 'Knab');
		$options['_']['items'][] = \JHtml::_('select.option', 'RABONL2U', 'Rabobank');
		$options['_']['items'][] = \JHtml::_('select.option', 'SNSBNL2A', 'SNS Bank');
		$options['_']['items'][] = \JHtml::_('select.option', 'ASNBNL21', 'ASN Bank');
		$options['_']['items'][] = \JHtml::_('select.option', 'RBRBNL21', 'Regio Bank');
		$options['_']['items'][] = \JHtml::_('select.option', 'TRIONL2U', 'Triodos Bank');
		$options['_']['items'][] = \JHtml::_('select.option', 'FVLBNL22', 'Van Lanschot Bankiers');
	...
	public function sendPayment(Gateway $jdideal)
	{
		...
		$fields['brq_currency'] = 'EUR';
		$fields['brq_amount'] = $details->amount;
		$fields['brq_invoicenumber'] = $details->order_number;
		$fields['brq_ordernumber'] = $details->order_number;
		...
		$fields['brq_description'] = substr(str_ireplace($find, $replace, $jdideal->get('description')), 0, 32);
		...
		if (!$request)
		{
			throw new \RuntimeException(\JText::_('COM_JDIDEALGATEWAY_BUCKAROO_CALL_FAILED'));
		}
		...
		elseif ($payment === 'sepadirectdebit')
		{
			\JFactory::getApplication()->enqueueMessage(
				urldecode($request['BRQ_CONSUMERMESSAGE_TITLE']) . '<br />' . urldecode($request['BRQ_CONSUMERMESSAGE_HTMLTEXT'])
			);
