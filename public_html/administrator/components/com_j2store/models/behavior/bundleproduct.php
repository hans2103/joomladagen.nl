...
class J2StoreModelProductsBehaviorBundleproduct extends F0FModelBehavior {

    private $_rawData = array();

    public function onAfterGetItem(&$model, &$record) {
        //sanity check
        if($record->product_type != 'bundleproduct') return;
        ...
        if($record->j2store_product_id) {
            ...
            try {
                ...
                $record->product_bundle = $record->params->get('bundleproduct',array());
    ...
    {
        if(!isset($data['product_type']) || $data['product_type'] != 'bundleproduct') return;
    ...
    public function onAfterSave(&$model) {
        if($this->_rawData) {
            ...
            if($table->product_type != 'bundleproduct') return;
    ...
    public function onBeforeDelete(&$model) {
        ...
        $product = F0FTable::getAnInstance('Product', 'J2StoreTable')->getClone();
        if($product->load($id)) {
            if($product->product_type != 'bundleproduct') return;
            $variantModel = F0FModel::getTmpInstance('Variants', 'J2StoreModel');
    ...
    public function onAfterGetProduct(&$model, &$product) {
        //sanity check
        if(!in_array($product->product_type, array('bundleproduct'))) return;
        ...
        if($product->app_params->get('manage_stock_based_on',1)){
            ...
            if(count ( $bundle_products ) > 0){
                foreach ($bundle_products as $single){
                    ...
                    if(!$single_product->variant->availability){
                        $product->variant->availability = 0;
                        break;
                    }
                }
