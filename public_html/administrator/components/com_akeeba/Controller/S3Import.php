...
class S3Import extends Controller
{
	...
	public function dltoserver()
	{
		...
		if ($part >= -1)
		{
			$this->container->session->set('s3import.part', $part, 'com_akeeba');
		}
		...
		if ($frag >= -1)
		{
			$this->container->session->set('s3import.frag', $frag, 'com_akeeba');
		}
		...
		if ($step >= -1)
		{
			$this->container->session->set('s3import.step', $step, 'com_akeeba');
		}
